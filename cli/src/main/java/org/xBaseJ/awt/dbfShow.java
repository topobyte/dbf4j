...
public class dbfShow extends Frame implements ActionListener, WindowListener,
		AdjustmentListener, ItemListener
{
	...
	memoDialog md;
	MenuItem opener, packer, quiter, firstRecord, nextRecord, prevRecord,
			lastRecord, addRecord, updateRecord, clearRecord;
	...
	public dbfShow(String title, String fname) throws Exception
	{
		...
		setTitle(dbname + ",   org.xBaseJ Version:"
				+ org.xBaseJ.DBF.xBaseJVersion);
		pack();
		setVisible(true);
	...
	public void setupDBFields(String dbname) throws Exception
	{
		...
		for (i = 1; i <= db.getFieldCount(); i++) {
			...
			if (f.isMemoField() || f.isPictureField()) {
				...
				addComponent(viewPane, t, 1, j, ln, 1,
						GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
		...
		addComponent(viewPane, crl, 0, i, 1, 1, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.EAST);
		addComponent(viewPane, trl, 1, i, 1, 1, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.EAST);
		i++;
		addComponent(viewPane, SBrecpos, 0, i, 2, 1,
				GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
		addComponent(viewPane, delCB, 2, i, 1, 1, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.EAST);
		i++;
		addComponent(viewPane, Prev, 0, i, 1, 1, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.EAST);
		addComponent(viewPane, Next, 1, i, 1, 1, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.EAST);
		i++;
		addComponent(viewPane, Add, 0, i, 1, 1, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.EAST);
		addComponent(viewPane, Update, 1, i, 1, 1,
				GridBagConstraints.HORIZONTAL, GridBagConstraints.EAST);
		addComponent(viewPane, Clear, 2, i, 1, 1, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.EAST);
	...
	public void goTo(int where)
	{
		try {
			...
			Next.setEnabled(
					!(db.getCurrentRecordNumber() == db.getRecordCount()));
			nextRecord.setEnabled(
					!(db.getCurrentRecordNumber() == db.getRecordCount()));
			...
			SBrecpos.setValues(db.getCurrentRecordNumber(), 1, 0,
					db.getRecordCount());
		} // try
