	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		if (ae.getSource() == menuOpen) {
			...
			try {
				...
				JOptionPane.showMessageDialog(this, xex.getMessage(),
						"xBaseJException Occurred", JOptionPane.ERROR_MESSAGE);
				repaint();
				return;
			} catch (IOException ioex) {
				JOptionPane.showMessageDialog(this, ioex.getMessage(),
						"IOException Occurred", JOptionPane.ERROR_MESSAGE);
				repaint();
				return;
			}
		...
		if (ae.getSource() == menuCreate) {
			...
			try {
				...
				JOptionPane.showMessageDialog(this, nfex.getMessage(),
...
class dbfCreateModel extends AbstractTableModel
{
	...
	*/
	private static final long serialVersionUID = 1L;
	int inRow;
	String columnName[] = { "Name", "Type", "Length", "Dec. Position",
			"Indexed", "Index Name" };
	int columnCount = 6;
	int rowCount;
	Boolean deleted[];
	...
	public void setValueAt(Object in, int r, int c)
	{
		...
		case 1:
			...
			String s = (String) in;
			if ((s.compareTo("Logical") == 0) || (s.compareTo("Memo") == 0)
