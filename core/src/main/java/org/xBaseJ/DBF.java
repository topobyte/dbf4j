...
public class DBF extends Object
{
	...
	protected void openDBF(String DBFname) throws IOException, xBaseJException
	{
		...
		if ((version != DBFTypes.DBASEIII)
			...
			String mismatch = Util.getxBaseJProperty("ignoreVersionMismatch")
					.toLowerCase();
			if (mismatch != null && (mismatch.compareTo("true") == 0
					|| mismatch.compareTo("yes") == 0)) {
		...
		if (MDX_exist == 1) {
			try {
				...
				String missing = Util.getxBaseJProperty("ignoreMissingMDX")
						.toLowerCase();
				if (missing != null && (missing.compareTo("true") == 0
						|| missing.compareTo("yes") == 0)) {
					MDX_exist = 0;
				} else {
	...
	public void addField(ArrayList<Field> aField)
			throws xBaseJException, IOException
	{
	...
	public void addField(Field aField[]) throws xBaseJException, IOException
	{
		...
		if (ignoreDBFLength != null && (ignoreDBFLength.toLowerCase()
				.compareTo("true") == 0
		...
		if (createTemp) {
			...
			for (i = 1; i <= fldcount; i++) {
				try {
					...
					throw new xBaseJException(
							"Clone not supported logic error");
				}
	...
	public void write(boolean lock) throws xBaseJException, IOException
	{
		...
		if (MDX_exist != 1 && (version == DBFTypes.DBASEIII
				|| version == DBFTypes.DBASEIII_WITH_MEMO)) {
			buffer.position(0);
			channel.write(buffer);
			wb = ' ';
			...
			throws xBaseJException, ArrayIndexOutOfBoundsException
	{
		...
		for (i = 0; i < fldcount; i++) {
			tField = fld_root.elementAt(i);
			if (name.toUpperCase()
					.compareTo(tField.getName().toUpperCase()) == 0) {
				return tField;
			}
		} /* endfor */
	...
	protected Field read_Field_header() throws IOException, xBaseJException
	{
		...
		default:
			throw new xBaseJException(
					"Unknown Field type '" + type + "' for " + name);
		} /* endswitch */
	...
	public void getXML(PrintWriter pw) throws IOException, xBaseJException
	{
		...
		int i;
		pw.println("<dbf name=\"" + Util.normalize(getName()) + "\" encoding=\""
				+ getEncodingType() + "\">");
		Field fld;
