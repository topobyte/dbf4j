...
public class DBT_iii extends DBTFile
{
	...
	public DBT_iii(DBF iDBF, boolean readOnly)
			throws IOException, xBaseJException
	{
		super(iDBF, readOnly, DBFTypes.DBASEIII_WITH_MEMO);
	}

	public DBT_iii(DBF iDBF, String name, boolean destroy)
			throws IOException, xBaseJException
	{
			...
			byte originalPos[]) throws IOException, xBaseJException
	{
		...
		if (value.length() == 0) {
			byte breturn[] = { BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO,
					BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO };
			return breturn;
		}

		if ((originalSize == 0) && (value.length() > 0)) {
			madebigger = true;
		} else if (((value.length() / memoBlockSize)
				+ 1) > ((originalSize / memoBlockSize) + 1)) {
			madebigger = true;
		} else {
			madebigger = false;
		}
		...
		for (pos = 0; pos < value.length(); pos += memoBlockSize) {
			...
			if ((pos + memoBlockSize) > value.length()) {
				try {
					b = value.substring(pos, value.length())
				...
				try {
					b = value.substring(pos, (pos + memoBlockSize))
							.getBytes(DBF.encodedType);
				} catch (UnsupportedEncodingException UEE) {
