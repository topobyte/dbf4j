...
public class MDX extends Index
{
	...
	public MDX(MDXFile ifile, DBF iDBF, short ipos)
...

	{
		...
		if ((i < 1) || (i > 10)) {
			throw new xBaseJException(
					"Invalid tag name " + iname + " name length incorrect");
		}
			...
			throws IOException, xBaseJException
	{
		...
		workNode = new MNode(mfile, key_per_Node, key_length, keyType, top_Node,
				level > 0);
		...
		btLoop: while (true) {
			if (pos == key_per_Node || bt == null) {
				...
				if (bt != null || tree2 != null) {
					if (tree2 == null) {
						topNode = workNode;
						tree2 = new BinaryTree(lastKey,
								workNode.get_record_number(), tree2);
					} else {
	...
	@Override
	public int find_entry(NodeKey findKey, int rec)
			throws xBaseJException, IOException
	{
		...
		for (aNode.set_pos(0); aNode.get_pos() < until; aNode.pos_up()) {
			...
			if (leaf > 0) { /* still dealing with Nodes */
				if (aNode.get_next() == null) {
					Node_2 = new MNode(mfile, key_per_Node, key_length, keyType,
							leaf, true);
	...
	@Override
	public int add_entry(NodeKey NDXkey, int recno)
			throws xBaseJException, IOException
	{
	...
	private void splitNode(MNode aNode, int savepos)
			throws xBaseJException, IOException
	{
		int i, j, k;
		int left, right;
		...
		bNode = new MNode(mfile, key_per_Node, key_length, keyType, 0,
				aNode.isBranch());
		bNode.set_pos(0);
		aNode.set_pos(0);
		...
		if (savepos > i) {
			...
			if (aNode.get_prev() == null) {
				...
				topNode.set_next(aNode);
					 ...
					 * create new second half and use the first half to put new
					 * key in
					 */
