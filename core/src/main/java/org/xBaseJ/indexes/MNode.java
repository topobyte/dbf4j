		...
		for (i = 0; i < keys_in_a_Node; i++) {
			...
			if (keyType == 'F') {
				...
				key_expression[i] = new NodeKey(
						new Double(Double.longBitsToDouble(nfile.readLong())));
			} else {
				nfile.readFully(key_buffer, 0, key_expression_size);
				for (k = 0; k < key_expression_size
						&& key_buffer[k] != 0; k++) {
					;
				}
				try {
					key_expression[i] = new NodeKey(
							new String(key_buffer, 0, k, DBF.encodedType));
		...
		for (i = 0; i < keys_in_a_Node && key_expression[i] != null; i++) {
			if (key_expression[i] == null) {
				throw new xBaseJException(
						"Missing Node Key expression at " + i);
			}
			...
			if (key_expression[i].getType() == 'F') {
				...
				byte bytebuffer[];
				try {
					bytebuffer = key_expression[i].toString()
