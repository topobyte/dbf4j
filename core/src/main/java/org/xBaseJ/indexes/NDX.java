...
public class NDX extends Index
{
			...
			throws IOException, xBaseJException
	{
		...
		while (reading > 0) {
			if (topNode == null) {
				...
				Node llNode = new Node(nfile, key_per_Node, key_length, keyType,
						Index_record, false);
	...
	private int reIndexWork(BinaryTree bt, int level)
			throws IOException, xBaseJException
	{
	...
	private void splitNode(Node aNode, int savepos)
			throws xBaseJException, IOException
	{
		...
		bNode = new Node(nfile, key_per_Node, key_length, keyType, 0,
				aNode.isBranch());
		bNode.set_pos(0);
		aNode.set_pos(0);
		...
		if (savepos > i) {
			...
			if (aNode.get_prev() != null) {
				bNode.set_pos(i - 1);
				update_entry(aNode.get_prev(), bNode.get_key_value(),
			...
			left = aNode.get_record_number();
			if (aNode.get_prev() != null) {
				update_entry(aNode.get_prev(), aNode.get_key_value(), left,
						right, 0);
			} else {
				bNode.set_pos(j - 1);
